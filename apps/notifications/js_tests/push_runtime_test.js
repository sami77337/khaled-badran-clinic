...
test("fixed AR/EN copy, no private content, approved icon and visible re-notification", async () => {
    const sw = worker();
    for (const event of ["new-consultation", "new-booking"]) {
        for (const language of ["ar", "en"]) {
            for (let i = 0; i < 2; i++) {
                ...
                assert.equal(notification.icon, "/static/img/brand/KB_APPROVED_PWA_192.png");

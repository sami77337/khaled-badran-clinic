...
class DashboardPatientRecordResponsiveContractTests(TestCase):
    ...
    def test_record_styles_define_mobile_single_column_and_overflow_contracts(self):
        ...
        for contract in (
            ...
            ".record-section-summary::marker",
            'content: "";',
            "grid-template-columns: minmax(0, 1fr);",
        ):
            with self.subTest(contract=contract):
                self.assertIn(contract, css)
        self.assertNotIn(".record-section-chevron", css)
    ...
    def test_major_record_sections_are_independent_native_details_closed_by_default(self):
        ...
        for section_id, english_label, arabic_label in expected_sections:
            with self.subTest(section=section_id):
                ...
                self.assertRegex(
                    english_content,
                    rf'<details[^>]+id="{re.escape(section_id)}"[^>]*>\s*'
                    r'<summary class="record-section-summary">',
                )
                self.assertNotRegex(
                    english_content,
...
class DashboardSchedulingTests(DashboardRecordWorkflowMixin, TestCase):
    ...
    def test_navigation_preserves_context_and_mobile_progressive_default_is_safe(self):
        ...
        scheduling_stylesheet = (
            project_root / "static" / "css" / "dashboard-scheduling.css"
        ).read_text(encoding="utf-8")
        self.assertIn("#customize-day-form", scheduling_stylesheet)
        self.assertIn("scroll-margin-block-start: 5.25rem;", scheduling_stylesheet)
